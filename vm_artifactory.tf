resource "azurerm_public_ip" "abd-de-ip-artifactory" {
  name = "abd-de-ip-artifactory"
...
resource "azurerm_network_interface" "abd-de-nic-artifactory" {
  name = "abd-de-nic-artifactory"
  location = "northcentralus"
  resource_group_name = azurerm_resource_group.abd-de-rg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.abd-de-ip-artifactory.id
  }

  tags = var.abd-de-tags
}

resource "azurerm_virtual_machine" "abd-de-vm-artifactory" {
    name = "abd-de-vm-artifactory"
    location = "northcentralus"
    resource_group_name = azurerm_resource_group.abd-de-rg.name
    network_interface_ids = [azurerm_network_interface.abd-de-nic-artifactory.id]
    vm_size = "Standard_D2s_v3"

    storage_os_disk {
        name = "abd-de-vm-artifactory-OS-disk"
        caching = "ReadWrite"
