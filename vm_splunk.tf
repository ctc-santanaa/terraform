resource "azurerm_public_ip" "abd-de-ip-splunk" {
  name = "abd-de-ip-splunk"
...
resource "azurerm_network_interface" "abd-de-nic-splunk" {
  name = "abd-de-nic-splunk"
  location = "northcentralus"
  resource_group_name = azurerm_resource_group.abd-de-rg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.abd-de-ip-splunk.id
  }

  tags = var.abd-de-tags
}

resource "azurerm_virtual_machine" "abd-de-vm-splunk" {
    name = "abd-de-vm-splunk"
    location = "northcentralus"
    resource_group_name = azurerm_resource_group.abd-de-rg.name
    network_interface_ids = [azurerm_network_interface.abd-de-nic-splunk.id]
    vm_size = "Standard_D2s_v3"

    storage_os_disk {
        name = "abd-de-vm-splunk-OS-disk"
        caching = "ReadWrite"
