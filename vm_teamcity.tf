resource "azurerm_public_ip" "abd-de-ip-teamcity" {
  name = "abd-de-ip-teamcity"
...
resource "azurerm_network_interface" "abd-de-nic-teamcity" {
  name = "abd-de-nic-teamcity"
  location = "northcentralus"
  resource_group_name = azurerm_resource_group.abd-de-rg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.abd-de-ip-teamcity.id
  }

  tags = var.abd-de-tags
}

resource "azurerm_virtual_machine" "abd-de-vm-teamcity" {
    name = "abd-de-vm-teamcity"
    location = "northcentralus"
    resource_group_name = azurerm_resource_group.abd-de-rg.name
    network_interface_ids = [azurerm_network_interface.abd-de-nic-teamcity.id]
    vm_size = "Standard_D4s_v3"

    storage_os_disk {
        name = "abd-de-vm-teamcity-OS-disk"
        caching = "ReadWrite"
