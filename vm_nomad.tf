resource "azurerm_public_ip" "abd-de-ip-nomad" {
  name = "abd-de-ip-nomad"
...
resource "azurerm_network_interface" "abd-de-nic-nomad" {
  name = "abd-de-nic-nomad"
  location = "northcentralus"
  resource_group_name = azurerm_resource_group.abd-de-rg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.abd-de-ip-nomad.id
  }

  tags = var.abd-de-tags
}

resource "azurerm_virtual_machine" "abd-de-vm-nomad" {
    name = "abd-de-vm-nomad"
    location = "northcentralus"
    resource_group_name = azurerm_resource_group.abd-de-rg.name
    network_interface_ids = [azurerm_network_interface.abd-de-nic-nomad.id]
    vm_size = "Standard_D2s_v3"

    storage_os_disk {
        name = "abd-de-vm-nomad-OS-disk"
        caching = "ReadWrite"
